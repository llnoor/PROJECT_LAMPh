...
#define NAME  "COM_SR850"
#define DEVICE "SR850"
#define COMMANDS "SR850,*IDN?,SR850"; //nameofdevice,KEYcommand,respond
...
#define INFO "The Lib for LAMPh to connect with SR850";
...
private:
    ...
    int i_test=0;
    QByteArray data;
    QSerialPort serialPortSR850;


public:
    ClassLAMPh( /*QString comPort_, QString comInfo_*/ ) // конструктор класса
    {
    //serialPortSR850 = QSerialPort();
        //connect(serialPortSR850, SIGNAL(readyRead()),this, SLOT(readData()));
    ...
    void readData()
    {
        while (! serialPortSR850.atEnd()) {
                QByteArray dataByteArray = serialPortSR850.read(100);
    ...
    bool setCOM( const char* const port ){        
        serialPortSR850.setPortName(port);
        serialPortSR850.setBaudRate(QSerialPort::Baud9600);
        serialPortSR850.setStopBits(QSerialPort::OneStop);
        serialPortSR850.setDataBits(QSerialPort::Data8);
        serialPortSR850.setParity(QSerialPort::NoParity);
        serialPortSR850.setFlowControl(QSerialPort::NoFlowControl);
        serialPortSR850.open(QIODevice::ReadWrite);

        if (serialPortSR850.isOpen()) return true; else false;
    }
    ...
    float getFloat(){
        serialPortSR850.write("00005555AA");
        //readData();

        //serialPortSR850.waitForBytesWritten(500);
        serialPortSR850.waitForReadyRead(500);
        QByteArray data = serialPortSR850.readAll();
