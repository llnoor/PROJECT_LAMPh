...
#define NAME  "LAMPh_USB_AGILENT"  	//Name of device
#define DEVICE "AGILENT" 			//Name of device
...
public:
    ...
    bool connectL(){
        ...
        viOpen (defaultRM, "USB0::0x0957::0x1707::my50002601::0::INSTR", VI_NULL,VI_NULL, &vi);
        ...
        agilentQString = QString("IDN:\n %1").arg(buf);
        //printf ("Instrument identification string: %s\n", buf);

        viPrintf (vi, "DISP:TEXT \"CONNECTED WITH LAMPh\"\n");
	...
	void readData(){
        char buf [256] = {0};
        viPrintf (vi, "MEAS:FREQ? DEF,DEF,(@2)\n");
        viScanf (vi, "%t", &buf);
        /*qDebug() << buf;
        for (int i=0; i<256; i++)
        {
        ...
        //double result_floatA_double = QString("%1").arg(buf).toDouble()*0.001;
        //result_floatA = result_floatA_double;
        result_floatA = QString("%1.%2%3%4").arg(buf[1]).arg(buf[3]).arg(buf[4]).arg(buf[5]).toFloat();
        result_floatB = QString("%1%2%3.%4%5%6").arg(buf[3]).arg(buf[4]).arg(buf[5]).arg(buf[6]).arg(buf[7]).arg(buf[8]).toFloat();
        result_floatC = QString("%1%2%3.%4%5%6").arg(buf[6]).arg(buf[7]).arg(buf[8]).arg(buf[9]).arg(buf[10]).arg(buf[11]).toFloat();
    }
	...
	const char* getRowName(int row){
		switch(row){
        case(0): return agilentQString.toStdString().c_str(); break;
